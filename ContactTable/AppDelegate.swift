...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func addToContext(name: String, photo: UIImage?, notes: String?) -> PersonsMO {
        let person = PersonsMO(context: persistentContainer.viewContext)
        person.name = name
        if let photo = photo {
            person.photo = UIImagePNGRepresentation(photo) as NSData?
        }
        person.notes = notes
    ...
    // 使用 PersonsMO 内嵌的 fetchRequest() 功能
    func fetchContext() -> [PersonsMO]? {
        var fetchedList:[PersonsMO]? = nil
        
        do {
            fetchedList = try persistentContainer.viewContext.fetch(PersonsMO.fetchRequest()) as? [PersonsMO]
        } catch {
    ...
    // 删除数据
    func deleteFromContext(person: PersonsMO) {
    ...
    // 更新数据
    func updateToContext(person: PersonsMO, name: String, photo: UIImage?, notes: String?) {
        person.name = name
        if let photo = photo {
            person.photo = UIImagePNGRepresentation(photo) as NSData?
        }
        ...
        saveContext()
        // 这个方法在用户点击save button 的时候才update
    }
